...
function findSpanTargetPath(nodeInEditorValue, offset, editorValue, block) {
  ...
  var count = 0;
  var targetKey; // If the parent is an inline, we must calculate a new start count,
  // as inlines are not modelled in Portable Text, and the count will be off

  if (nodeInEditorValueParent.object === 'inline') {
    var texts = editorValue.document.getParent(nodeInEditorValueParent.key).getTexts();
    var previousNumberOfTexts = texts.findIndex(node => node.key === nodeInEditorValue.key);
    var previousTexts = texts.slice(0, previousNumberOfTexts);
    var leavesCount = previousTexts.reduce((acc, current) => {
...
function createOperationToPatches(blockContentFeatures, blockContentType) {
  ...
  function removeNodePatch(operation, beforeValue, afterValue) {
    var patches = [];
    var block = toBlock(beforeValue, operation.path.get(0));
    var oldNode = beforeValue.document.getNode(operation.path);
    var isPlaceholder = oldNode && oldNode.data && oldNode.data.get('placeholder'); // Don't create any patches if this is a placeholder block that is removed.
